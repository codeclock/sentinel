...
def has_suppo_conf():
    ...
    valid_suppo_conf = False

    # ensure suppo_conf exists & readable
    #
    # if not, print a message stating that Suppo Core must be installed and
    # configured, including JSONRPC access in suppo.conf
    try:
        f = io.open(config.suppo_conf)
        valid_suppo_conf = True
    except IOError as e:
        print(e)

    return valid_suppo_conf
...
def main():
    ...
    if not has_suppo_conf():
        print("SuppoCore must be installed and configured, including JSONRPC access in suppo.conf")
        sys.exit(1)
