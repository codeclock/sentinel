...
def is_valid_suppo_address(address, network='mainnet'):
    ...
    suppo_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Suppo address
        return False

    if (address_version != suppo_version):
        return False
...
# shims 'til we can fix the suppod side
def SHIM_serialise_for_suppod(sentinel_hex):
    from models import SUPPOD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SUPPOD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in suppod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # suppod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    suppod_hex = serialise(obj)
    return suppod_hex


# shims 'til we can fix the suppod side
def SHIM_deserialise_from_suppod(suppod_hex):
    from models import SUPPOD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(suppod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('suppo.conf')
