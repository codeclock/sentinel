...
class GovernanceObject(BaseModel):
    ...
    # sync suppod gobject list with our local relational DB backend
    @classmethod
    def sync(self, suppod):
        golist = suppod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_suppod(suppod, item)
    ...
    @classmethod
    def import_gobject_from_suppod(self, suppod, rec):
        import decimal
        import suppolib
        import inflection
        ...
        # shim/suppod conversion
        object_hex = suppolib.SHIM_deserialise_from_suppod(object_hex)
        objects = suppolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from suppod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from suppod, with every run
        ...
        # get/create, then sync payment amounts, etc. from suppod - Suppod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(suppod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from suppod! %s" % e)
            govobj.vote_delete(suppod)
            return (govobj, None)
    ...
    def vote_delete(self, suppod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(suppod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, suppod, signal, outcome):
        import suppolib
        ...
        output = suppod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = suppolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(suppod, signal)

    def sync_network_vote(self, suppod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = suppod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SUPPOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 suppo addr, non-multisig
            if not suppolib.is_valid_suppo_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Suppo address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (suppolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SuppoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import suppolib
        obj_data = suppolib.SHIM_serialise_for_suppod(self.serialise())
    ...
    def prepare(self, suppod):
        try:
            object_hash = suppod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SUPPOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not suppolib.is_valid_suppo_address(addr, config.network):
    ...
    def hash(self):
        import suppolib
        return suppolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SUPPOD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, suppod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - suppod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, suppod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - suppod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, suppod):
        now = int(time.time())
        return (self.created_at < (now - suppod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, suppod):
        if self.is_expired(suppod):
            return False

        return True

    def is_deletable(self, suppod):
        if self.is_expired(suppod):
            return True
