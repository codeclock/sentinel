...
class SuppoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some suppod methods
    ...
    @classmethod
    def from_suppo_conf(self, suppo_dot_conf):
        from suppo_config import SuppoConfig
        config_text = SuppoConfig.slurp_config_file(suppo_dot_conf)
        creds = SuppoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = suppolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = suppolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = suppolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_suppod_proto_version_with_sentinel_ping)
