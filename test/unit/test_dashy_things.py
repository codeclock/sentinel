...
def test_valid_suppo_address():
    from suppolib import is_valid_suppo_address

    main = valid_suppo_address()
    test = valid_suppo_address('testnet')

    assert is_valid_suppo_address(main) is True
    assert is_valid_suppo_address(main, 'mainnet') is True
    assert is_valid_suppo_address(main, 'testnet') is False

    assert is_valid_suppo_address(test) is False
    assert is_valid_suppo_address(test, 'mainnet') is False
    assert is_valid_suppo_address(test, 'testnet') is True


def test_invalid_suppo_address():
    from suppolib import is_valid_suppo_address

    main = invalid_suppo_address()
    test = invalid_suppo_address('testnet')

    assert is_valid_suppo_address(main) is False
    assert is_valid_suppo_address(main, 'mainnet') is False
    assert is_valid_suppo_address(main, 'testnet') is False

    assert is_valid_suppo_address(test) is False
    assert is_valid_suppo_address(test, 'mainnet') is False
    assert is_valid_suppo_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from suppolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % suppolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(suppolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(suppolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(suppolib.blocks_to_seconds(16616)) == 2612035
